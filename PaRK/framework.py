...
class MyFrame():
    ...
    def forward(self, volatile=False):
        ...
        self.img = V(self.img, volatile=volatile)  # Remove .cuda()
        if self.mask is not None:
            self.mask = V(self.mask, volatile=volatile)  # Remove .cuda()
        if self.if_key_points is not None:
            self.if_key_points = V(self.if_key_points, volatile=volatile)  # Remove .cuda()
        if self.all_key_points_position is not None:
            self.all_key_points_position = V(self.all_key_points_position, volatile=volatile)  # Remove .cuda()
        if self.anchor_link is not None:
            self.anchor_link = V(self.anchor_link, volatile=volatile)  # Remove .cuda()
