...
def default_loader(id, root):
    img_path = os.path.join(root, '{}_sat.png').format(id)
    mask_path = os.path.join(root, '{}_mask.png').format(id)
    mat_path = os.path.join(root, '{}_mask.mat').format(id)
    
    img = cv2.imread(img_path)
    mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
    key_points = scipy.io.loadmat(mat_path)
