...
imageID = 0
for root, dirs, files in os.walk("preprocess/key_points_final"):
    for file in files:
        imageID = imageID + 1
        key_points_info = scipy.io.loadmat("preprocess/key_points_final/" + file)
        if_key_points = key_points_info["if_key_points"]
        all_key_points_position= key_points_info["all_key_points_position"]

        mask = cv2.imread("preprocess/scribble/" + file[:-4] + ".png")
        img = mask[:,:,0]
        ...
        final_mat_savepath = "preprocess/link_key_points_final/" + file
